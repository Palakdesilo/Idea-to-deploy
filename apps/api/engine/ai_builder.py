...
class AIBuilder:
    ...
    async def build_project(self, project_id: str, description: str) -> Dict[str, Any]:
        ...
        functional_doc = next((d for d in docs if d.category == 'REQUIREMENTS'), None) # Using REQUIREMENTS as FUNCTIONAL equivalent
        ...
        # README
        screen_links = '\n'.join([f"- **{s}**: [http://localhost:3000/{s.lower().replace(' ', '-') or ''}](http://localhost:3000/{s.lower().replace(' ', '-') or ''})" for s in screens])
...
"""
        ...
        for screen in screens:
            ...
            # Try to extract content from UI/UX doc
            section = ""
            if uiux_doc:
                matches = re.findall(rf'### {screen} Screen([\s\S]*?)(?=###|$)', uiux_doc.content, re.IGNORECASE)
                if matches:
                    section = matches[0]
            
            screen_components = []
            comp_match = re.search(r'- \*\*Components\*\*: (.*)', section)
            if comp_match:
                screen_components = [c.strip() for c in comp_match.group(1).split(',')]

            screen_purpose = "Main interface for " + screen
            purpose_match = re.search(r'- \*\*Purpose\*\*: (.*)', section)
            if purpose_match:
                screen_purpose = purpose_match.group(1)
                
            interactions = "Standard user interactions"
            int_match = re.search(r'- \*\*Interactions\*\*: (.*)', section)
            if int_match:
                interactions = int_match.group(1)
            
            nav_links = "{%s}" % ', '.join([f"""<a key="{s}" href="/{s.lower().replace(' ', '-')}" className="hover:text-indigo-600 transition-colors">{s}</a>""" for s in screens])
...
export default function {screen.replace(' ', '')}Page() {{
  const screens = {json.dumps(screens[:4])};
  const screenComponents = {json.dumps(screen_components)};
  const features = {json.dumps(features[:5])};
...
"""
            page_path = f"apps/web/app/{'' if slug == 'dashboard' else slug}/page.tsx"
            files.append({
                "path": page_path,
        ...
        # 3. Backend Implementation (Express + TS) - Simplified for brevity but functional
        files.append({
...
"""
        ...
        feature_endpoints = "\n".join([f"""
// {f} endpoint
featureRouter.get('/{f.lower().replace(' ', '-')}', (req, res) => {{
    res.json({{ 
        success: true,
        project: '{description}',
        feature: '{f}',
...
""" for f in features])
        ...
        files.append({
            "path": "apps/api/src/routes/features.ts",
            "content": f"""
...
""" for f in features if f not in ['Login', 'Logout', 'Registration']])
...
"""
        ...
        return {
            ...
            "stats": {
                "fileCount": len(files),
                "totalLines": sum([len(f["content"].splitlines()) for f in files])
            }
        }
